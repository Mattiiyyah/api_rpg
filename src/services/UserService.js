...
class UserService {
  ...
  async updateUserSkill(requesterRole, requesterId, targetUserId, skillId, newLevel) {
    const isKing = requesterRole === 'KING';
    const isMaster = requesterRole === 'MASTER';
    const isSelf = Number(targetUserId) === requesterId;

    if (!isKing && !isMaster && !isSelf) {
      throw { status: 401, errors: ['Você não tem permissão para alterar as habilidades de outro aventureiro.'] };
    }
    ...
    if (userId.role === 'KING' && !isSelf) {
      throw { status: 401, errors: ['Apenas o Rei pode alterar suas próprias habilidades.'] };
    }

    if (userId.role === 'MASTER' && isMaster && !isSelf) {
      throw { status: 401, errors: ['Um Mestre não pode alterar as habilidades de outro Mestre.'] };
    }
    ...
    if (!userSkill) {
      throw { status: 404, errors: ['Você precisa aprender essa habilidade antes de evoluí-la.'] };
    }

    // Regra de negócio: Level máximo 100
    if (newLevel > 100) {
      throw { status: 400, errors: ['O nível máximo permitido é 100.'] };
    }

    if (newLevel < 0) {
      throw { status: 400, errors: ['Não é possível diminuir o nível de uma habilidade abaixo de 0.'] };
    }

    // Regra de negócio: Apenas aumentar nível (exceto Rei/Mestre que podem editar livremente?)
    // Vamos assumir que a rota é "update", então setar o nível direto é o esperado.
    // Mas se o usuário comum chamar, ele provavelmente só quer aumentar +1.
    // O controller recebe "nivel" do body. Vamos confiar que o frontend manda o valor correto (atual + 1).
